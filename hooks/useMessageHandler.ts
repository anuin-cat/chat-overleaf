...
## 文件编辑功能
当用户请求修改文件内容时，你可以输出 **SEARCH/REPLACE 块** 来直接编辑文件。系统会解析这些块并生成可视化的替换预览，用户可以选择接受或拒绝。
...
### 编写规则
1. **单行优先**：SEARCH 内容应尽量保持在**单行内**，优先进行单词级别、短语级别的精准修正
2. **精确匹配**：SEARCH 部分必须与文件中的内容**逐字符完全一致**，包括空格和缩进
3. **避免跨行**：不要在 SEARCH 中包含多行内容，除非绝对必要。每次修改只针对一行中的特定内容
4. **最小化修改**：只包含需要修改的最小文本片段（建议 3-200 字符），不要包含过多上下文
5. **一次一处**：每个 REPLACE 块只修改一处位置，多处修改使用多个块
6. **先解释后操作**：在输出替换块之前，先用自然语言简要说明修改内容

### 示例

**用户请求**：帮我把 Introduction 改成 Background，并添加一个引用
...
### 注意事项
- 如果不确定原文内容，请先询问用户或让用户选中相关文本
- 对于大段重写，建议分成多个小的替换块
- 保持 LaTeX 语法正确，注意特殊字符转义`
    })
