...
### 三、创建文件操作（CREATE_FILE）
用于新建文件，并写入文件内容（若文件夹不存在，会自动创建）。
...
**核心规则：**
1. **必须提供路径和内容**：FILE 与 CONTENT 均不能为空
2. **路径使用相对路径**：例如 \`chapters/introduction.tex\`
3. **不要用于覆盖已有文件**：若文件已存在，请改用 REPLACE/INSERT
